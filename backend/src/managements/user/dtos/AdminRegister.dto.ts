import { IsEmail, IsNotEmpty, IsPhoneNumber, IsString, MaxLength, MinLength, IsArray, ArrayNotEmpty, IsIn } from 'class-validator'
...
@IsString({ message: 'Tên đăng nhập phải là kí không phải hình đặc biệt' })
 ...
 @IsString({ message: 'Mật khẩu phải là chuỗi không phải hình đặc biệt ' })
  ...
  @IsArray()
  @ArrayNotEmpty()
  @IsIn(['user', 'admin'], { each: true ,message:'Vai trò 1 trong admin hoặc user'})
  roles: string[]
}
